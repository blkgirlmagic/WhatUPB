    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 403) {
          setError(data.error || "Message blocked — threats and abuse aren't allowed. Try rephrasing.");
  ...
  if (sent) {
    return (
      <div className="anim-2 profile-main-card" style={{ textAlign: "center", padding: "40px 28px" }}>
        ...
        <button onClick={() => { setSent(false); setCoinTicker(""); setSignalType(null); }} style={{ padding: "10px 24px", borderRadius: "50px", border: "1px solid var(--faint)", background: "rgba(255,255,255,0.6)", color: "var(--muted)", fontFamily: "var(--font-lora), 'Lora', Georgia, serif", fontSize: "14px", cursor: "pointer" }}>
  ...
  if (showCrisisIntercept) {
    return (
      <div className="anim-2 profile-main-card" style={{ textAlign: "center", padding: "40px 24px" }}>
        ...
        <p style={{ color: "var(--ink2)", fontSize: "15px", lineHeight: 1.6, marginBottom: "24px" }}>{crisisInterceptMessage}</p>
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          <a href="tel:988" className="card-btn-primary" style={{ textAlign: "center", textDecoration: "none" }}>Call or Text 988</a>
          <a href="https://988lifeline.org/chat/" target="_blank" rel="noopener noreferrer" style={{ display: "block", width: "100%", padding: "13px", borderRadius: "12px", border: "1px solid rgba(59,130,246,0.25)", background: "rgba(59,130,246,0.08)", color: "#2563eb", fontSize: "14px", textAlign: "center", textDecoration: "none" }}>
            Chat Online Now
          </a>
          <button type="button" onClick={handleCrisisInterceptDismiss} style={{ width: "100%", padding: "13px", borderRadius: "12px", border: "none", background: "transparent", color: "var(--muted)", fontSize: "14px", cursor: "pointer" }}>
            Go Back
          </button>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmitAttempt} className="anim-2 profile-main-card">
        ...
        {prompt && (
          <div style={{ marginBottom: "20px" }}>
            <div style={{ fontSize: "10.5px", letterSpacing: "2.5px", textTransform: "uppercase", color: "#c9a84c", fontWeight: 500, marginBottom: "6px" }}>They want to know:</div>
            <div style={{ fontSize: "16px", fontStyle: "italic", fontWeight: 400, color: "var(--lav)", lineHeight: 1.5 }}>&ldquo;{prompt}&rdquo;</div>
          </div>
        )}

        {/* Coin ticker input */}
        <div style={{ marginBottom: "14px" }}>
          <div style={{ fontSize: "10px", letterSpacing: "2px", textTransform: "uppercase", color: "var(--muted)", fontWeight: 500, marginBottom: "6px" }}>
          ...
          <input
            ...
            style={{
              ...
              border: "1px solid rgba(155,142,232,0.18)", background: "rgba(255,255,255,0.6)",
              fontFamily: "var(--font-ibm-plex-mono), 'IBM Plex Mono', monospace",
              fontSize: "14px", fontWeight: 600, color: "#9B8EE8",
        ...
        <div style={{ marginBottom: "16px" }}>
          <div style={{ fontSize: "10px", letterSpacing: "2px", textTransform: "uppercase", color: "var(--muted)", fontWeight: 500, marginBottom: "8px" }}>
        ...
        <textarea
          value={content}
          onChange={(e) => { setContent(e.target.value); setCrisisAcknowledged(false); }}
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "6px", fontSize: "12.5px", color: "var(--muted)" }}>
          <span style={{ fontSize: "13px" }}>{"lock"}</span>
          Completely anonymous. Signal updates rep score instantly.
        </div>
      </form>

      {showCrisisModal && (
        <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 16px" }}>
          <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.5)", backdropFilter: "blur(4px)" }} onClick={() => setShowCrisisModal(false)} />
          <div style={{ position: "relative", zIndex: 10, width: "100%", maxWidth: "380px", background: "rgba(255,255,255,0.92)", backdropFilter: "blur(40px)", border: "1px solid rgba(255,255,255,0.95)", borderRadius: "22px", padding: "28px 24px" }}>
            ...
            <p style={{ color: "var(--muted)", fontSize: "14px", textAlign: "center", lineHeight: 1.6, marginBottom: "24px" }}>
              ...
              <a href="tel:988" style={{ color: "var(--lav)", textDecoration: "underline", fontWeight: 500 }}>988</a> anytime.
            </p>
            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
              <a href="tel:988" className="card-btn-primary" style={{ textAlign: "center", textDecoration: "none" }}>Call or Text 988</a>
              <button type="button" onClick={handleCrisisContinue} style={{ width: "100%", padding: "13px", borderRadius: "12px", border: "1px solid var(--faint)", background: "transparent", color: "var(--muted)", fontSize: "14px", cursor: "pointer" }}>
                Continue Sending
              </button>
            </div>
          </div>
